...
export default function KpiPage() {
  ...
  const calculateKpi = async () => {
    ...
    try {
      // Fetch the predefined file from the public folder
      const response = await fetch('/BASES/2025/julio.xlsx');
      if (!response.ok) {
    ...
    <div className="flex min-h-screen w-full items-center justify-center bg-gray-100 dark:bg-gray-900">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>
            Calcula el número de gestantes con captación oportuna (Edad Gestacional de Inicio de Control &lt; 10) desde el archivo predefinido.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          <Button onClick={calculateKpi} className="w-full" disabled={isLoading}>
