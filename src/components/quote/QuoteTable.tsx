...
import { urlToPngDataURL } from "@/lib/image";
...
export function QuoteTable({ items }: QuoteTableProps) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      const logoDataUrl = await urlToPngDataURL(logoUrl);
      ...
      (doc as any).autoTable({
          ...
          didDrawPage: function (data: any) {
              // Header
              doc.addImage(logoDataUrl, 'PNG', data.settings.margin.left, 15, 20, 20);
              doc.setFontSize(16);
              doc.setTextColor(40);
      ...
      toast({
        ...
        description: "No se pudo cargar la imagen del logo para el PDF. Por favor, inténtelo de nuevo.",
      });
    } finally {
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
            ...
            <div className="flex flex-col items-end gap-4">
                ...
                <Button id="download-pdf-btn" onClick={handleDownloadPdf} disabled={isGeneratingPdf}>
                    {isGeneratingPdf ? (
